...
import { CodeBlock, LinkIcon, classifyLinkPath } from '@deepseek-ai/dsh-client-ui-primitives'
import type {
  MarkdownFenceRenderer, MarkdownFileMentions, MarkdownLabels, MarkdownPathImages,
} from '../contract/slots.ts'
import type { PositionedBlock } from './incremental.ts'
import css from './MarkdownText.module.css'

export type { MarkdownCodeLabels, MarkdownFileMentions, MarkdownLabels, MarkdownPathImages } from '../contract/slots.ts'
...
export interface MarkdownRenderContext {
  ...
  /** Fence and math dispatch for this surface; null sends every node back to the renderer's own fallback. */
  readonly renderFence: MarkdownFenceRenderer
...
function renderNode(node: Md.RootContent, key: Key, context: MarkdownRenderContext): ReactNode {
  switch (node.type) {
    ...
    case 'math':
      // A rule that claims display math renders it; otherwise the authored TeX
      // stays literal, matching how an unclosed delimiter already renders.
      return <Fragment key={key}>{context.renderFence({ kind: 'display', source: node.value }, node.value)}</Fragment>
    case 'inlineMath':
      return <Fragment key={key}>{context.renderFence({ kind: 'inline', source: node.value }, node.value)}</Fragment>
    case 'list':
...
function renderCode(node: Md.Code, key: Key, context: MarkdownRenderContext): ReactNode {
  ...
  if (context.streaming) {
    // Fences stay source while the reply streams: an incomplete fence cannot
    // render, and re-rendering per chunk would repeat the parse for every
    // intermediate state. They still go through the seat so the settled pass
    // reconciles the same outlet and the code block keeps its DOM.
    return (
      <Fragment key={key}>
        {context.renderFence({ kind: 'fence', lang, info: node.lang ?? '', source: node.value, streaming: true }, codeArm)}
      </Fragment>
    )
  }
  // The settled pass offers the fence to the surface's fence seat, handing it
  // this code arm as the chain's fallback: a language no rule claims renders
  // exactly as a language with no special renderer does.
  return (
    <Fragment key={key}>
      {context.renderFence({ kind: 'fence', lang, info: node.lang ?? '', source: node.value, streaming: false }, codeArm)}
    </Fragment>
  )
}
