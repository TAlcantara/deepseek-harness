...
import { parseGfm, parseGfmWithMath } from '@deepseek-ai/dsh-client-ui-primitives'
...
import type { MarkdownFenceRenderer } from '../contract/slots.ts'
...
class StreamingRenderer {
   ...
   * @param renderFence - Fence dispatch baked into cached elements; a new identity also replaces the renderer.
   */
  constructor(
    private readonly labels: MarkdownLabels,
    private readonly renderFence: MarkdownFenceRenderer,
  ) {}
 ...
 * must not bake in handlers that could go stale. `renderFence` dispatches a
 * settled fence and every math node to the owning surface's fence seat;
 * pass a reference-stable function for the same reason as `labels`.
 * @returns A GFM document with raw HTML, relative links, and unsafe protocols
 * disabled and absolute HTTP(S) images rendered directly; fences and math
 * nodes carry whatever `renderFence` returns, or the renderer's own fallback.
 */
export const MarkdownText = memo(function MarkdownText({ text, streaming = false, labels, fileMentions, pathImages, renderFence }: {
  text: string
  streaming?: boolean
  labels: MarkdownLabels
  fileMentions?: MarkdownFileMentions | undefined
  ...
  const streamSeatRef = useRef<{ labels: MarkdownLabels; renderFence: MarkdownFenceRenderer }>({
    labels, renderFence,
  })
  const children = useMemo(() => {
    if (!streaming) {
      streamRef.current = null
      return renderSettled(text, labels, fileMentions, pathImages, renderFence)
    }
    const seat = streamSeatRef.current
    if (streamRef.current === null || seat.labels !== labels || seat.renderFence !== renderFence) {
      streamRef.current = new StreamingRenderer(labels, renderFence)
      streamSeatRef.current = { labels, renderFence }
    }
    return streamRef.current.render(text)
  }, [text, streaming, labels, fileMentions, pathImages, renderFence])
