...
`ui-primitives` stays a static library and keeps what is not composition: the two mdast grammars (`parseGfm`, `parseGfmWithMath`), the plain-text projection (`extractMarkdownPlainText`), Shiki highlighting, `CodeBlock`, and `JsonBlock`. It no longer exports `MarkdownText` or the markdown label types.
...
Two rules ship with the plugin. The Mermaid rule claims a fence whose normalized language token is `mermaid`. The math rule claims an `inline` or `display` math node and a settled `math` fence, and typesets each through KaTeX.
...
**Have each surface register the built-in rules itself.** That would remove the concentrated registration cost, but `ui-primitives` cannot host the rule components and no feature plugin may runtime-import another feature plugin's values. It would also duplicate the math and Mermaid rules once per surface and let the surfaces' rendering drift.
...
The diagram arm carries its own copy of the code-block fence shell in `DiagramBlock.module.css`, because a plugin cannot import another package's stylesheet. The two sheets must stay byte-compatible, and the copy is the price of the diagram's banner, copy button, and canvas sitting in the same message as ordinary fences.
...
`ui-primitives` shrinks to what has no composition: the grammars, the plain-text projection, highlighting, and the code and JSON blocks. Surfaces that need markdown now depend on the plugin row, and their markdown chrome comes from the shared namespace instead of their own dictionaries.

## Verification

Unit specs in `packages/client/ui-markdown/tests` cover the renderer, the built-in rules, the incremental parser, path-image rewriting, and hand-built mdast trees. `markdown-dom-parity.client.spec.tsx` pins the rendered DOM byte-for-byte against `tests/fixtures/markdown-dom`, so the markup cannot drift.
