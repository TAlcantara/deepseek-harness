...
**Theme.** Mermaid bakes concrete colors into its SVG and refuses CSS functions in `themeVariables` — it runs each value through a color library that rejects `var(--dsw-…)` during `initialize()`. It does pass `themeCSS` through verbatim into the SVG's own stylesheet, and that sheet resolves custom properties in the live document. Diagram paints therefore name `--dsw-*` tokens with `!important` (Mermaid scopes its own rules by the render id, which outranks plain class selectors), and a light/dark switch repaints every diagram without a re-render. A class Mermaid puts on both a box and the label inside it must be element-qualified in each rule — `.actor` is `rect.actor` and `text.actor`, so naming it bare in both the surface rule and the text rule leaves the box wearing the text color and hides its label. Chart families whose colors encode categories rather than surface — pie, mindmap, timeline, gitgraph, sankey, radar, treemap, xychart — keep the library palette.
...
`tests/mermaid.client.spec.tsx` replaces the lazy `import('mermaid')` with a stub, because jsdom cannot lay out SVG text, and covers the streaming gate, the viewport gate, the config, failure fallback, cache reuse and eviction, per-instance id rewriting, unmount races, and copy behavior. Two blind spots follow from that stub, and each carries its own guard. The specs cannot observe the placeholder's box, so one spec reads `DiagramBlock.module.css` and pins the placeholder's `display` declaration: the viewport observer watches that element, and a boxless one reports a 0x0 rect that never intersects, which leaves every diagram on the code arm. The specs also cannot prove Mermaid draws, so that burden falls to the browser suite, where a fixture with `mermaid` fences belongs. `tests/markdown-dom-parity.client.spec.tsx` does not gain a `mermaid` case: a real load is nondeterministic, and the existing corpus contains no such fence, so no fixture drifts.
